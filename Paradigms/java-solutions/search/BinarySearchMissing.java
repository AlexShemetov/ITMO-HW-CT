...
//  Pred: l >= -1 && r <= arr.length && l < r && arr[i] <= arr[i + 1]
//  Post: min r` : arr[r`] = x || -(min r`) - 1 : arr[r`] < x
    public static int recursion(int[] arr, int x, int l, int r) {
        // l >= -1 && r <= arr.length && l < r
        // l >= -1 && r <= arr.length && r - l > 0
        // l >= -1 && r <= arr.length && r - l > 0 && arr.length >= 0
        // l >= -1 && r <= arr.length && r - l > 1
        if (r - l <= 1) {
            if (arr.length == r || arr[r] != x) {
                return -r - 1;
            }
            return r;
        }
        ...
        if (arr[mid] < x) {
            ...
            // arr[r] >= arr[mid] && arr[mid] >= x && r < arr.length || r >= arr.length
            // arr[r] <= x && r < arr.length || r >= arr.length
...
//  Post: min r` : arr[r`] = x || -(min r`) - 1 : arr[r`] < x
    public static int iterative(int[] arr, int x) {
        ...
        // l == -1 && r == arr.length && -1 < arr.length
        // l == -1 && r == arr.length && l < r
        // l == -1 && r == arr.length && r - l > 0 && r > 0 && l == -1
        // l == -1 && r == arr.length && r - l > 1
        while (r - l > 1) {
            // r > l && r <= arr.length && l >= -1
            int mid = (l + r) >> 1;
            // mid = (l + r) / 2 && r != l
            // mid = (l + r) / 2 && mid > l && mid < r
            if (arr[mid] < x) {
                ...
                r = mid;
                // r` == mid && r` < r
                // arr[r] >= arr[r`] && arr[r`] >= x && r < arr.length || r >= arr.length
                // arr[r] >= x && r < arr.length || r >= arr.length
            }
        }
