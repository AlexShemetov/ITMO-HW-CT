...
public class FreemarkerServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        Template template;
        try {
            String uri = URLDecoder.decode(request.getRequestURI(), UTF_8).replaceAll("/+", "/");
    ...
    private Map<String, Object> getData(HttpServletRequest request) {
        ...
        for (Map.Entry<String, String[]> e : request.getParameterMap().entrySet()) {
            if (e.getValue() != null && e.getValue().length == 1) {
                if (e.getKey().endsWith("_id")) {
                    try {
                        data.put(e.getKey(), Integer.valueOf(e.getValue()[0]));
                    } catch(Exception igon) {
                        data.put(e.getKey(), -1);
                    }
                } else {
        ...
        data.put("currPage", request.getRequestURI());
